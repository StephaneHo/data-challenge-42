...
def native_score(df: pd.DataFrame, pred_col="pred", gt_col="target",
    ...
    """
    mean_err = weighted_err(df[pred_col].values,df[gt_col].values)
...
def reweighted_score(df: pd.DataFrame, target_bin_probs: np.ndarray,
    ...
    mean_err = (len(F)*err_F + len(M)*err_M) / (len(F)+len(M))
    return {
        "err_female": err_F,
        "err_male": err_M,
        "mean_err": mean_err,
...
def evaluate_predictions(df: pd.DataFrame, pred_col="pred", gt_col="target",
    ...
    if verbose:
        ...
        print()
        print(f"{'distribution':<20} {'err_F':>10} {'err_M':>10} {'mean_err':>10} {'gap':>10} {'score':>10}")
        print(f"{'-' * 65}")
        s = out["native_val"]
        print(f"{'val natif (empiric)':<20} {s['err_female']:>10.5f} {s['err_male']:>10.5f} {s['err_score']:>10.5f} "
              f"{s['gap']:>10.5f} {s['score']:>10.5f}")
        for name in ["brief", "spread", "heavy"]:
            s = out[f"reweighted_{name}"]
            print(f"{'reweighted ' + name:<20} {s['err_female']:>10.5f} {s['err_male']:>10.5f} {s['mean_err']:>10.5f} "
                  f"{s['gap']:>10.5f} {s['score']:>10.5f}")
        print()
